...
Need a quick way to exercise every tool without wiring up an MCP client? Run the local dashboard:

```bash
npm run ui
# UI_HOST=0.0.0.0 UI_PORT=4173 npm run ui  # override defaults
```

The command spins up an Express server that serves `public/index.html` plus JSON endpoints under `/api`:

- `POST /api/speedrun-plan`
- `POST /api/clip-outline`
- `POST /api/analyze-docs`
- `POST /api/greet`

Each card in the UI maps to one of the tools, includes sample payloads, and renders responses inline so you can validate flows from a browser hitting `http://localhost:4173`.
